...
print('programa iniciado: light GBM')
...
# Convertir datos a formato Dataset de LightGBM
...
param_grid = {
    'num_leaves': [None, 200],
    'n_estimators': [70, 100],  # Número de árboles en el bosque
    ...
    'subsample': [0.4]   # Submuestreo de la muestra para entrenar cada árbol
}

# Configuración del modelo
gbm_model = lgb.LGBMClassifier(boosting_type='gbdt', objective='multiclass', metric='multi_logloss', num_class=3, random_state=42)

# Realizar la búsqueda de hiperparámetros utilizando GridSearchCV
grid_search = GridSearchCV(estimator=gbm_model, param_grid=param_grid, cv=StratifiedKFold(n_splits=4, shuffle=True, random_state=42),
...
plt.savefig('conf_matrix_lightGBM')
plt.show()
plt.close()
...
# Mostrar el gráfico
plt.tight_layout()
plt.savefig('hist_accuracy_lightGBM')
plt.show()
plt.close()
